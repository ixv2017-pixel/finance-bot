...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👋 **أهلاً بك في حاسبة التمويل**\nيرجى اختيار المنتج:", reply_markup=ReplyKeyboardMarkup(kb, one_time_keyboard=True, resize_keyboard=True), parse_mode='Markdown')
    return PRODUCT

async def product_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['product'] = update.message.text
    kb = [['Saudi', 'Expats']]
    await update.message.reply_text("الجنسية:", reply_markup=ReplyKeyboardMarkup(kb, one_time_keyboard=True, resize_keyboard=True))
    return NATIONALITY
...
async def nationality_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [['ST', 'NST']]
    await update.message.reply_text("نوع التحويل (ST: راتب محول | NST: غير محول):", reply_markup=ReplyKeyboardMarkup(kb, one_time_keyboard=True, resize_keyboard=True))
    return ST_NST
...
async def type_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("القطاع الوظيفي:", reply_markup=ReplyKeyboardMarkup(kb, one_time_keyboard=True, resize_keyboard=True))
    return SECTOR

async def sector_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['sector'] = update.message.text
    await update.message.reply_text("💰 أدخل **صافي الراتب** (أرقام فقط، مثال: 10000):", reply_markup=ReplyKeyboardRemove(), parse_mode='Markdown')
    return SALARY

async def salary_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        context.user_data['salary'] = float(update.message.text)
        await update.message.reply_text("💳 إجمالي الالتزامات الشهرية (0 إذا لا يوجد):")
        return OBLIGATIONS
    except ValueError:
        await update.message.reply_text("⚠️ الرجاء إدخال أرقام فقط للراتب.")
        return SALARY
...
async def obligations_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("📅 مدة التمويل بالأشهر (مثال: 60):")
        return TENURE
    except ValueError:
        await update.message.reply_text("⚠️ الرجاء إدخال أرقام فقط للالتزامات.")
        return OBLIGATIONS
...
async def tenure_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        msg = (f"✅ **ملخص التمويل المقترح**\nــــــــــــــــــــــــــــــــــــــــــــــــ\n🔹 **التمويل الصافي:** `{principal:,.2f}` ريال\n🔹 **القسط الشهري:** `{install:,.2f}` ريال\n🔹 **النسبة المطبقة:** `{used_rate*100:.2f}%`\n🔹 **إجمالي السداد:** `{total:,.2f}` ريال\n🔹 **الأرباح:** `{profit:,.2f}` ريال\nــــــــــــــــــــــــــــــــــــــــــــــــ\nاضغط /start لحساب جديد")
        await update.message.reply_text(msg, parse_mode='Markdown')
        return ConversationHandler.END
    except ValueError:
        await update.message.reply_text("⚠️ الرجاء إدخال رقم صحيح للمدة.")
        return TENURE
